...
class DEGraphWin(tk.Canvas):
    ...
    def setOffsets(self, x_offset, y_offset):
    ...
    def getMouseAsync(self):
        '''Wait for mouse click and return Point object from the click'''
        '''Usage warning: you must reset clickPoint to None at the end of your loop'''
        self.update()
    ...
    def grid(self, dx, dy, color='black'):
...
class DropDown(GraphicsObject):
    ...
    def setValue(self, value):
        if value in self.choices:
            ...
            raise ValueError(f"Value {value} not in dropdown choices: {self.choices}")
...
class Slider(GraphicsObject):
    ...
    def setValue(self, value):
        if self.slider:
            ...
            raise RuntimeError("Slider has not been drawn yet")
    def getValue(self):
...
class RuleDropDown(GraphicsObject):
    def __init__(self, topLeft, choices, label, font=('courier', 18), bg='black'):
        GraphicsObject.__init__(self, [])
        self.anchor = topLeft.clone()
        self.text = tk.StringVar(_root)
        self.text.set(choices[0])
        self.label = label
        self.fill = "gray"
        self.color = "black"
        self.bgColor = bg
    ...
    def getChoice(self):
        return self.text.get()
class RuleCheckBox(GraphicsObject):
    def __init__(self, topLeft=Point(0, 0), choices=[], label="", font=('courier', 18), bg='black', callback=None):
        GraphicsObject.__init__(self, [])
        self.anchor = topLeft.clone()
        self.label = label
        self.fill = "gray"
    ...
    def _draw(self, canvas, options):
        frm = tk.Frame(canvas.master)
        self.vars = [tk.IntVar(value=0) for _ in self.choices]

        # Create a label for the checkboxes group
        label = tk.Label(frm, text=self.label, font=self.font, bg=self.bgColor)
        label.pack()

        # Create vertical checkboxes
        for i, choice in enumerate(self.choices):
            cb = tk.Checkbutton(frm, text=choice, variable=self.vars[i], font=self.font, bg=self.bgColor, command=self._on_check)
            cb.pack(anchor=tk.W)
    ...
    def getSelections(self):
        return [int(choice) for i, choice in enumerate(self.choices) if self.vars[i].get() == 1]

    def setSelections(self, selections):
        for i, choice in enumerate(self.choices):
            if int(choice) in selections:
                self.vars[i].set(1)
            else:
                self.vars[i].set(0)
